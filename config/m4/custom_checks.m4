...
AC_DEFUN([LFP_SYS_PLATFORM_SPECIFIC_DEFINITIONS], [
 case "$host_os" in
  linux*)
    LFP_CFLAGS="$LFP_CFLAGS -D_ISOC11_SOURCE=1 -D_XOPEN_SOURCE=700 -D_GNU_SOURCE=1"
    ;;
  freebsd*)
    LFP_CFLAGS="$LFP_CFLAGS -D_ISOC11_SOURCE=1 -D_XOPEN_SOURCE=700 -D__BSD_VISIBLE=1"
    ;;
  netbsd*)
    LFP_CFLAGS="$LFP_CFLAGS -D_ISOC11_SOURCE=1 -D_XOPEN_SOURCE=700 -D_NETBSD_SOURCE=1"
    ;;
  openbsd*)
    LFP_CFLAGS="$LFP_CFLAGS -D_ISOC11_SOURCE=1 -D__BSD_VISIBLE=1"
    ;;
  darwin*)
    LFP_CFLAGS="$LFP_CFLAGS -D_XOPEN_SOURCE=700 -D_DARWIN_C_SOURCE=1"
    ;;
  sunos* | solaris*)
    LFP_CFLAGS="$LFP_CFLAGS -D_XOPEN_SOURCE=700 -D__EXTENSIONS__ -D_POSIX_PTHREAD_SEMANTICS"
    ;;
 esac
])
